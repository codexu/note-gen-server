...
本文面向 NoteGen 客户端维护者，说明如何把 `note-gen-server` 接入桌面端和移动端。它用于指导客户端实现、交互设计和验收，不替代 `/openapi.json` 的字段定义。
...
界面显示服务端返回的名称、版本和连接安全状态。非本机地址使用明文 HTTP 时应阻止继续；证书错误不能提供静默忽略选项。
...
登录成功后不展示 Workspace 选择，客户端直接调用 `POST /v1/workspaces/default`：

- 账号还没有默认同步空间时，服务端原子创建并保存托管密钥 envelope。
- 已存在时返回同一个内部同步空间，避免重复创建。
- 客户端将当前 NoteGen 笔记目录标识与该连接保存在本机，随后自动设为主要同步方式。
- 本地目录发生变化时暂停同步，防止将两个资料库意外混入同一远端空间；用户在新目录重新连接后才重新绑定。

内部同步空间 ID 只用于协议和数据隔离，不出现在普通设置界面。不同设备的绝对路径可以不同，它们通过同一账号的默认同步空间关联，不能把本机绝对路径作为服务端数据主键。
...
旧版本创建的 E2EE 默认空间不能阻塞普通同步。若客户端资料未标记为用户主动启用高级 E2EE，服务端保留旧空间及其密文、取消其默认标记，并创建新的托管默认空间；客户端随后把当前本地 Markdown 快照自动同步到新空间。只有用户明确选择恢复旧 E2EE 数据时才要求口令或恢复密钥。
