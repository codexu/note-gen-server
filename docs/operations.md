...
首次打开 Web 页面会进入安装向导。向导负责设置实例名称，并在同一事务中创建首位实例管理员。保存后服务会自动切换到完整运行状态。官方公共测试服务与用户自行部署使用同一套独立实例运行方式。
...
数据库连接、公开 URL 和认证密钥属于启动配置，继续保留在 env。注册策略、邀请和可选 SMTP 由实例管理员在 Web 后台维护。

## 公共测试实例边界

官方公共实例与自行部署使用相同程序，不加载计费、订阅、Staff 运营后台或商业客服能力。它仅用于免费测试和社区体验，不承诺商业 SLA，也不应作为用户数据的唯一备份。

实例管理员仍需承担最低限度的维护工作：保护管理员凭据、默认关闭或限制注册、处理明显滥用、设置合理的上传与存储上限、监控磁盘容量，并定期备份 PostgreSQL 与 Blob 数据。SMTP 是可选项；未配置邮件时，管理员可以直接复制一次性邀请链接给用户。
...
空数据库启动后只装配安装页面、健康检查和安装 API。访问 Web 页面并完成向导。服务会在安装响应结束后自动关闭安装态监听、重新读取数据库配置并装配完整服务；页面检测到就绪后自动进入普通登录入口。空实例不要求额外安装密钥，因此应先完成安装，再开放公网入口。
...
服务只维护独立实例这一种运行方式，不区分官方服务与自行部署。同步协议、镜像和数据库结构也不分叉。历史运营模式数据库不会自动转换，需要先迁移业务数据或使用新的空数据库。
...
完整的执行顺序和责任边界见 [公共测试实例上线清单](production-checklist.md)。
