...
> 现行产品只维护“独立实例”一种运行方式：官方公共测试服务与自行部署使用相同的账号、管理员和运维边界。本文及子文档中关于 hosted、Staff 运营域、订阅、计费和平台客服的内容仅保留为历史设计记录，不再属于当前交付计划。

- 状态：历史设计归档；当前仅继续维护独立实例相关能力
- 日期：2026-08-11
- 面向读者：NoteGen Server、账号 Web、NoteGen 客户端的开发者与部署维护者
- 文档目标：保留既有设计决策背景；当前实现范围以独立实例为准

## 1. 结论

下一阶段不继续扩张同步协议。同步、设备、加密、实时通知、后台管理和基础备份继续作为共享核心；新增能力只围绕独立实例的账号生命周期和可维护性展开。

产品固定为一种运行形态：用户名登录，首次初始化后默认关闭注册，可按需启用邀请或公开注册；邮件与 SMTP 可选；不加载 Staff 运营域、商业配额、订阅、支付、平台客服或官方托管专属控制面。官方公共测试服务也按这一形态部署。

数据库中的 `deploymentMode` 暂时只作为历史迁移字段保留，不再是产品功能开关。
