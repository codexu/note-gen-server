...
export async function unlockManagedWorkspaceKeys(
  ...
  for (const version of versions) {
    ...
    keys.set(version.keyVersion, await crypto.subtle.importKey(
      "raw",
      bytes,
      { name: "AES-GCM" },
      false,
...
export async function decodeWorkspaceObject(
  ...
  if (!key) {
    return { object, payload: null, decryptionError: `缺少密钥 v${object.keyVersion}` }
  }
  try {
    ...
    return { object, payload: null, decryptionError: "密文无法解密或内容格式不受支持" }
  }
}
...
export function contentTitle(item: DecodedSyncObject): string {
  ...
  if (item.object.kind === "note") {
    ...
    return path ? path.split("/").at(-1)?.replace(/\.md$/i, "") || path : fallbackTitle(item)
  }
...
export function contentSummary(item: DecodedSyncObject): string {
  ...
  if (item.object.kind === "canvas") {
    ...
    const entries = [payload.nodes, payload.elements, payload.items, payload.shapes, document?.nodes]
      .find((value) => Array.isArray(value))
    if (Array.isArray(entries)) return `绘图包含 ${entries.length} 个元素`
  }

  if (item.object.kind === "setting") {
    return compact(JSON.stringify(sanitizePayload(payload)) ?? "配置内容为空", 180)
  }
...
export function displayPayload(item: DecodedSyncObject): string {
  if (item.decryptionError) return item.decryptionError
  const payload = recordPayload(item.payload)
  if (item.object.kind === "note" && payload && typeof payload.content === "string") {
...
function sanitizePayload(value: unknown, key = ""): unknown {
  if (/password|passphrase|token|secret|api[-_]?key|access[-_]?key/i.test(key)) {
    return value === undefined || value === null || value === "" ? value : "••••••••"
  }
  if (Array.isArray(value)) return value.map((item) => sanitizePayload(item))
  if (typeof value === "object" && value !== null) {
    return Object.fromEntries(Object.entries(value).map(([entryKey, entryValue]) => (
      [entryKey, sanitizePayload(entryValue, entryKey)]
    )))
  }
  return value
}
