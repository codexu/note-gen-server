...
import { devicePairings } from '../database/schema.js'
...
export class DevicePairingService {
  ...
  async create(accountId: string): Promise<DevicePairingResult> {
    const now = new Date()
    await this.database.db.delete(devicePairings).where(and(
    ...
    const [pairing] = await this.database.db.insert(devicePairings).values({
  ...
  async exchange(pairingToken: string, input: DeviceSessionInput): Promise<SessionResult> {
    ...
    const [pairing] = await this.database.db.update(devicePairings).set({ consumedAt }).where(and(
      ...
      isNull(devicePairings.consumedAt),
    )).returning({ id: devicePairings.id, accountId: devicePairings.accountId })
    if (pairing === undefined) {
      throw new ApiError({
    ...
    try {
      ...
      return await this.auth.createDeviceSession(pairing.accountId, input)
    } catch (error) {
